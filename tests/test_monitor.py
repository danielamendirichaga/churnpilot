...
from churnpilot.monitor import DriftReport, monitor_drift
...
def panel():
    ...
    return make_panel(n_subscribers=2000, n_months=18, seed=61)
...
def test_flags_the_drifting_feature(panel):
    ...
    # watch_hours_30d drifts on purpose → should be flagged.
    assert "watch_hours_30d" in report.drifted
    watch = next(r for r in report.features if r["feature"] == "watch_hours_30d")
    assert watch["psi"] > 0.25 and watch["status"] == "major"


def test_stable_feature_not_flagged(panel):
    report = monitor_drift(panel, _cfg())
    age = next(r for r in report.features if r["feature"] == "age")
    assert age["psi"] < 0.1  # age is not engineered to drift
    assert "age" not in report.drifted
